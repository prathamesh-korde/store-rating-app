...
import {
  LayoutDashboard, Users, Store, UserPlus, LogOut, Menu, X, Star,
  ChevronRight, Bell, Calendar, User, Shield, ChevronDown
} from 'lucide-react';
...
const NOTIFICATIONS = [
  { id: 1, msg: 'New user registered on the platform', time: '2m ago', unread: true },
  { id: 2, msg: 'A store rating was submitted', time: '15m ago', unread: true },
  { id: 3, msg: 'Platform stats updated', time: '1h ago', unread: false },
];
...
export function AdminLayout() {
  ...
  /* NavLink active logic: exact match for items that need `end` */
    ...
    <div className="main-layout bg-slate-100 min-h-screen">
      ...
      <aside className={`sidebar ${sidebarOpen ? 'open' : ''}`}>
        <div className="px-6 py-5 flex items-center gap-3 border-b border-white/5">
          <div className="w-9 h-9 rounded-xl bg-indigo-500 flex items-center justify-center shadow-glow flex-shrink-0">
        ...
        <nav className="flex-1 px-3 pt-4 overflow-y-auto" aria-label="Admin navigation">
          <div className="space-y-0.5">
            {navSections.flatMap(s => s.items).map(({ to, label, icon: Icon, end }) => (
              <NavLink
                key={to}
                to={to}
                end={end}
        ...
        <header className="sticky top-0 z-30 bg-white/90 backdrop-blur-md border-b border-slate-200 px-4 sm:px-6 py-3 flex items-center gap-3 shadow-sm">
          ...
          <div className="flex items-center gap-2 ml-auto">
            ...
            <div className="relative" ref={notifRef}>
              ...
              {showNotif && (
                <div className="absolute right-0 top-full mt-2 w-80 bg-white rounded-2xl shadow-2xl border border-slate-200 overflow-hidden z-50 animate-scaleIn">
                  ...
                  <div className="divide-y divide-slate-100 max-h-72 overflow-y-auto">
                    {notifications.map((n) => (
                      <div key={n.id}
                        className={`flex items-start gap-3 px-4 py-3 hover:bg-slate-50 transition-colors cursor-pointer ${n.unread ? 'bg-indigo-50/60' : ''}`}>
                        <div className={`w-2 h-2 rounded-full mt-1.5 flex-shrink-0 ${n.unread ? 'bg-indigo-500' : 'bg-slate-300'}`} />
                  ...
                  <div className="px-4 py-2.5 border-t border-slate-100 bg-slate-50">
                    <p className="text-xs text-center text-slate-400">Platform notifications</p>
                  </div>
                </div>
              )}
            </div>
              ...
              {showProfile && (
                <div className="absolute right-0 top-full mt-2 w-56 bg-white rounded-2xl shadow-2xl border border-slate-200 overflow-hidden z-50 animate-scaleIn">
                  <div className="px-4 py-3 bg-gradient-to-br from-indigo-50 to-violet-50 border-b border-slate-100">
                    ...
                    <span className="mt-1.5 inline-flex items-center gap-1 px-2 py-0.5 bg-indigo-100 text-indigo-700 text-xs font-semibold rounded-full">
                      <Shield size={10} /> System Admin
                    </span>
                  </div>
                  <div className="p-1.5">
                    <button onClick={() => { setShowProfile(false); navigate('/admin/profile'); }}
